...
bool PICOXRMotionTracking::GetBodyTrackingSupported(bool& Supported, TArray<EPXRBodyTrackingMode>& SupportedModes)
{
	bool bResult = false;
	bool supported = false;
	int modesCount = 0;
	...
	for (int i = 0; i < modesCount; i++)
	{
		SupportedModes.Emplace(static_cast<EPXRBodyTrackingMode>(modes));
	}
...
bool PICOXRMotionTracking::StartBodyTracking(const FPXRBodyTrackingStartInfo& StartInfo)
{
	...
	info.apiVersion = PXR_EYE_TRACKING_API_VERSION;
	info.mode = static_cast<PxrBodyTrackingMode>(StartInfo.StartMode);
	info.BodyJointSet = StartInfo.OtherMode == -1 ? static_cast<int>(StartInfo.SwiftMode) : StartInfo.OtherMode;
	...
	if (FPICOXRVersionHelper::IsThisVersionOrGreater(0x2000308))
	{
		...
		if (bResult)
		{
			for (int i = 0; i < 24; i++)
			{
				...
				PXR_LOGV(PxrUnreal, "PXR_GetBodyTrackingPose LocalPose index:%d:Location %s,Rotation %s", i,
				         *BodyTrackingData.RoleDatas[i].LocalPose.GetLocation().ToString(),
				         *BodyTrackingData.RoleDatas[i].LocalPose.GetRotation().ToString());

				PXR_LOGV(PxrUnreal, "PXR_GetBodyTrackingPose GlobalPose index:%d:Location %s,Rotation %s", i,
...
bool PICOXRMotionTracking::StartBodyTrackingCalibApp(FString CalibFlagString, int CalibMode)
{
	bool bResult = false;
...
bool PICOXRMotionTracking::GetMotionTrackerLocations(float WorldToMetersScale, const FString& trackerSN, FPXRMotionTrackerLocations& locations)
{
	bool bResult = false;
	...
	bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().GetMotionTrackerLocations(dPredictTime,TCHAR_TO_ANSI(*trackerSN),&MotionTrackerLocations));
	if (bResult)
	{
